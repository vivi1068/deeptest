#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 18-3-28 下午10:37
# @Author  : gonghuihui
# @File    : http_client_pom_demo.py
import http.client
import logging
import unittest
...
class LYMLogging:
    def __init__(self,
        ...
        #定义一个StreamHeamHeandler,将INFO级别或更高的日志信息打印到标准错误
        # 并将其添加到当前的日志处理对象
...
class LYMHttp:
    ...
    def request(self,
        ...
        self.http.request(method=method, url=url, headers=headers)
...
class BookSearchPage(Page):
    ...
    def search_python_book(self, method, url, body=None, headers={}):
        self.request(method=method, url=url,body=None,headers=headers)
...
class TestSearchBookPage(unittest.TestCase):
    ...
    def test_search_python_book(self):
        # 查找python相关的书籍及q=python,只找两本书即count=2
